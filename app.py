...
hf_token = "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"
model_id = "Ankithar/deepseek-model"
...
def optimize_code():
    ...
    try:
        ...
        hf_url = "https://your-username-your-space-name.hf.space"  

        response = requests.post(
            hf_url,
            json={"code": code_input},  # depends on what input your Space expects
            timeout=30
        )

        if response.status_code == 200:
            result = response.json()
            optimized = result.get("optimized", "⚠️ No optimized code received.")
            return jsonify({'optimized': optimized})
        else:
            return jsonify({'optimized': f"Error from Hugging Face: {response.text}"}), 500

    except Exception as e:
        print("🔥 Optimization error:", e)
        return jsonify({'optimized': f"Error optimizing code: {e}"}), 500
